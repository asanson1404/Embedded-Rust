//! Module which prvides gamma correction to represent
//! colors as our eyes can see them.
//! This module contains a gamma table which works fine with our
//! let matrix

const GAMMA_TAB: [u8; 256] = [
    ,
];

/// Function which returns the corresponding value in the table. Arguments:
/// - 'x': the input value, the index of the tab
/// - return: the corrected value
pub fn gamma_correct(x: u8) -> u8 {
    GAMMA_TAB[x as usize]
}